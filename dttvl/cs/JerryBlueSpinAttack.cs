...
public class JerryBlueSpinAttack : AttackBase
{
	private Jerry jerry;
	...
	protected override void Update()
	{
		...
		else if (state == 1)
		{
			...
			if (frames >= 120)
			{
				if (frames % 10 == 0)
				{
					...
					UnityEngine.Object.Instantiate(slashPrefab, new Vector3(4.25f, num4), Quaternion.Euler(0f, 0f, UnityEngine.Random.Range(-182f, -178f))).GetComponent<JerrySlashBlue>().SetSpeed(6f);
				}
				if (frames % 40 == 0)
				{
				...
				for (int j = 0; j < platforms.Count; j++)
				{
					...
					if (platforms[j].transform.position.x <= 4.25f && safetyRangeRegistered[j] == 0)
					{
					...
					else if (platforms[j].transform.position.x <= -0.2f && safetyRangeRegistered[j] == 1)
					{
						safetyRangeRegistered[j]++;
		...
		else
		{
			...
			{
				platforms[k].transform.position -= new Vector3(Mathf.Lerp(3f, 10f, (float)(frames - 30) / 30f) / 48f, 0f);
			}
			frames++;
			if (frames <= 15)
			{
			...
			if (frames >= 50)
			{
				sword.GetComponent<SpriteRenderer>().color = new Color(0f, 0.23529412f, 1f, 1f - (float)(frames - 50) / 20f);
				if (frames == 70)
				{
					jerry.SetPose(0);
				}
				jerry.GetPart("sword").GetComponent<SpriteRenderer>().color = new Color32(0, 60, byte.MaxValue, (byte)(int)Mathf.Lerp(0f, 255f, (float)(frames - 70) / 20f));
			}
			if (frames == 90)
			{
