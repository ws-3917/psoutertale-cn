...
public class TitleScreen : TranslatableSelectableBehaviour
{
	...
	private static string[] controlNames = new string[7] { "Down", "Right", "Up", "Left", "确认", "取消", "Menu" };
	...
	public override Dictionary<string, string[]> GetDefaultStrings()
	{
		...
		dictionary.Add("title", new string[2] { "[press {0} or enter]", "Kris和Susie跑去了\n他俩不该待的地方" });
		...
		{
			"要删除这个存档吗？", "真的要删除吗？", "这个文件将被覆盖。", "是", "否", "Yes!", "No!", "[空白]", "[CORRUPTED]", "\b    THIS DATA IS CORRUPT\n\b OR UNREACHABLE. DO YOU WISH\n\b TO TERMINATE ITS CONNECTION?",
	...
	private void Awake()
	{
		...
		options = base.transform.Find("设置");
	...
	private void Update()
	{
		...
		else if (state == State.NameConfirm)
		{
			...
			else if (state == State.FileSelect)
			{
				if (saveState != SaveState.CopyOverwriteConfirm && saveState < SaveState.DeleteConfirm)
				{
					...
					{
						soul.localPosition = Vector3.Lerp(soul.localPosition, saveFiles.Find("其他").transform.localPosition + new Vector3(-20f, 16f), 0.4f);
					}
					else
					{
						if (index < 0 || index > 2)
					...
					if (UTInput.GetButtonDown("Z"))
					{
						...
						else if (saveIndex == 3)
						{
							...
							if (index == 0)
							{
								...
								saveFiles.Find("其他").GetComponent<Text>().enabled = false;
		...
		private void ResetSaveState()
		{
			...
			saveFiles.Find("其他").GetComponent<Text>().enabled = true;
		...
		public void LoadDefaultMenu()
		{
			...
			if (UTInput.joystickIsActive)
			{
				...
				string[] array2 = new string[3] { "确认", "取消", "Menu" };
				for (int i = 0; i < 3; i++)
				{
			...
			gamerules.Find("Rules").GetComponent<Text>().text = string.Format(GetString("instructions", 1), UTInput.GetKeyName("确认"), UTInput.GetKeyName("取消"), UTInput.GetKeyName("Menu"));
		...
		public void UpdateAllText()
		{
			...
			base.gameObject.transform.Find("Unresponsive").GetComponent<Text>().text = string.Format(GetString("title", 0), UTInput.GetKeyName("确认").ToLower());
			langPackMenu.Find("PackHeader").GetComponent<Text>().text = GetString("pack_menu", 0);
			langPackMenu.Find("Unresponsive").GetComponent<Text>().text = string.Format(GetString("pack_menu", 1), UTInput.GetKeyName("取消").ToLower());
			...
			letters.Find("返回").GetComponent<Text>().text = GetString("name_selection", 0);
			letters.Find("结束").GetComponent<Text>().text = GetString("name_selection", 1);
